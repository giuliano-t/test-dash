...
def update_output_container(selected_statistic, input_year):
    if selected_statistic == 'Recession Period Statistics':
        ...
        R_chart3 = dcc.Graph(
            figure=px.pie(values=exp_rec, names=exp_rec.index, title='Advertising Expenditure Share between vehicles')
            )

        # Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        hhhh = recession_data.groupby(['Vehicle_Type'])['Automobile_Sales'].mean()
        R_chart4 = dcc.Graph(
            figure=px.bar(hhhh,
    ...
    # Yearly Statistic Report Plots
        ...
        Y_chart1 = dcc.Graph(figure=px.line(x=yas['Year'],y=yas['Automobile_Sales'],title='Bla bla bla'))

        # Plot 2 Total Monthly Automobile sales using line chart.
        Y_chart2 = dcc.Graph(figure=px.line(x=yearly_data['Month'],y=yearly_data['Automobile_Sales'],title='Bla bla bla bla'))

        # Plot bar chart for average number of vehicles sold during the given year
        avr_vdata = yearly_data.groupby('Vehicle_Type')['Automobile sales'].mean()
        Y_chart3 = dcc.Graph(figure=px.bar(avr_vdata,
                             title='Average Vehicles Sold by Vehicle Type in the year {}'.format(input_year)))

        # Total Advertisement Expenditure for each vehicle using pie chart
        exp_data = yearly_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum()
        Y_chart4 = dcc.Graph(figure=px.pie(values=exp_data,names=exp_dat.index,title='Bla bla bla 3'))
